...
const TicketForm = () => {
  ...
  const submitHandler = async (e) => {
    ...
    const response = await TicketStore.buy({
      name: FCS.value,
  ...
  const formFields = (
    <div className="ticketForm__content">
      <Input name="FCS" type="text" input={FCS}>
        ФИО
      </Input>
      <Selector options={types} input={type} placeholder="Тип билета" />
    </div>
  );
  return (
    <form className={'ticketForm'} onSubmit={submitHandler}>
      ...
      {!!ticketId || (
        <Button type={'main'} disabled={!FCS.correct || !type.correct}>
