...
const EditForm = () => {
    ...
    const formFields = (
        <div className="ticketForm__content">
            <Input name="name" type='text' input={name}>Имя</Input>
            <Input name="specie" type='text' input={species}>ФИО</Input>
            <Selector
                options={enclosuresOptions}
                input={enclosure}
                placeholder="Тип билета"
            />
            <Input name="birth" type='text' input={birthDate}>Дата рождения</Input>
            <Input name="arrival" type='text' input={arrivalDate}>Дата прибытия</Input>
        </div>
    )
    return (
        <form className={'ticketForm'}
            ...
            <div className="ticketForm__buttons">
                <Button
                    ...
                    style={'full-width'}
                >
                    <Paragraph level={1} type={'bright'}>Сохранить</Paragraph>
                </Button>
                <Button
                    type={'sub'}
                    style={'full-width'}
